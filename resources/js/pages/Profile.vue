<template>
    <div>
        ...
        <form @submit.prevent="handleSubmit">
            ...
            <div class="form-group">
                <button type="button" class="btn btn-primary" :disabled="status.registering" id="passwordReset" @click="displayPasswordFields = !displayPasswordFields">Wachtwoord opnieuw instellen</button>

                <span v-if="displayPasswordFields">
                    <div class="form-group mt-3">
                        <label for="password">Nieuw wachtwoord</label>
                        <input type="password" autocomplete="new-password" id="password" v-model="user.password" required title="Gebruik ten minste 6 karakters" name="password" class="form-control" 
                        ...
                        <input type="password" autocomplete="new-password" id="password_confirmation" v-model="user.password_confirmation" required title="Gebruik ten minste 6 karakters" name="password_confirmation" class="form-control" 
                        :class="{ 'is-invalid': submitted && errors.hasOwnProperty('password') }" />
                        <div v-if="submitted && errors.hasOwnProperty('password')" class="invalid-feedback">{{ errors.password.join(' ') }}</div>
                    </div>
                </span>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary" :disabled="status.updating">Bijwerken</button>
                ...
                <router-link to="/learning_goals" class="btn btn-link">Terug</router-link>
            </div>
        </form>
    </div>
</template>
